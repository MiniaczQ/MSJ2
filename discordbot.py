...
class DiscordBot(discord.Client, DiscordBotCommands, DiscordBotReact, DiscordBotListen):
    ...
    async def on_ready(self):
        ...
        print(
            f"MSJ2 Discord Bot connected to #{channel.name} in {channel.guild.name}")
...
if __name__ == "__main__":
    ...
    root = getcwd()
    m = Manager(Mock(), logging, 12, root, path.join(root, 'templates'), 6, '192.168.1.2',
                25566, path.join(root, 'servers'), path.join(root, 'worlds'), 'fabric-server-launch.jar')
    m.servers = [MockServer(0), MockServer(1), MockServer(4)]
    load_dotenv()
